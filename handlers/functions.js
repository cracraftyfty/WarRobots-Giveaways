...
function onCoolDown(message, command) {
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     .setFooter(ee.footertext, ee.footericon)]}).catch(e => console.log(`[${config.console_tag}] THIS IS TO PREVENT A CRASH`))
     if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log(`[${config.console_tag}] THIS IS TO PREVENT A CRASH`))
